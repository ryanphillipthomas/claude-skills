...
`.trim();
export function summariseCaptures(records) {
    const byState = {};
    const byProvenance = {};
    const failures = [];
...
export async function runReport(args, logger) {
    ...
    if (args.flags.get('json') === true) {
        process.stdout.write(`${JSON.stringify({ ...summary, ...(reportPath === undefined ? {} : { reportPath }) }, null, 2)}\n`);
        return 0;
    }
    const lines = [

...
import { UiAtlasError, STATE_NAMES, STILL_CAPTURE_KINDS, } from '@ui-atlas/protocol';
import { flagString, requireHttpUrl } from '../args.js';
...
function parseKind(value) {
    if (value === undefined)
        return 'viewport';
    if (STILL_CAPTURE_KINDS.includes(value))
        return value;
    throw new UiAtlasError('config.invalid', `--kind must be one of ${STILL_CAPTURE_KINDS.join(', ')} (got "${value}")`);
}
...
export async function runCapture(args, logger) {
    ...
    try {
        ...
        for (const state of states) {
            ...
            if (record.status === 'captured')
                logger.info(`${summary} → ${record.image?.relativePath ?? ''}`);
            else
                logger.warn(`${summary}: ${record.error?.message ?? 'no detail'}`);
        }
    }
    finally {

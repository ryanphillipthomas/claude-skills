import { readStyles } from './page-scripts.js';
/**
...
export async function probeStyles(locator, timeoutMs = 5_000) {
...
export function diffStyles(before, after) {
    ...
    for (const [property, value] of Object.entries(after.styles)) {
        ...
        if (previous !== undefined && previous !== value)
            changed[property] = { from: previous, to: value };
    }
    const delta = { changed };
    if (before.visibleDescendants !== after.visibleDescendants)
        delta.descendantVisibilityChanged = true;
    if (Math.abs(before.box.width - after.box.width) > 0.5 ||
...
export function deltaHasEvidence(delta) {
    return (Object.keys(delta.changed).length > 0 ||
        delta.descendantVisibilityChanged === true ||
        delta.boundsChanged === true);
}
//# sourceMappingURL=style-diff.js.map
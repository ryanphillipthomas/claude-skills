...
import { blurActiveElement, blurElement, forceStateFlag, readStateFlags, undoStateFlag, } from './page-scripts.js';
import { PointerTracker, releaseModifiers } from './pointer.js';
import { probeStyles } from './style-diff.js';
function requireLocator(ctx, state) {
    if (ctx.locator === undefined) {
        throw new UiAtlasError('state.unsupported', `state "${state}" needs a selected element`);
    }
    return ctx.locator;
}
async function readFlags(locator, timeoutMs) {
    return locator.evaluate(readStateFlags, undefined, { timeout: timeoutMs });
}
function observed(name, verification, label) {
    const state = { name, provenance: 'observed', verified: true, verification };
    if (label !== undefined)
        state.label = label;
    return state;
}
 ...
 */
export async function applyState(ctx, name, label) {
    const steps = [];
    switch (name) {
        case 'default':
        case 'custom': {
...
async function safeProbe(locator, timeoutMs) {
    try {
...
async function applyHover(ctx, steps) {
    ...
    if (box !== null)
        ctx.pointer.note(box.x + box.width / 2, box.y + box.height / 2);
...
async function applyFocus(ctx, steps) {
 ...
 */
async function applyFocusVisible(ctx, steps) {
    ...
    const application = {
        ...
        steps,
        cleanup: NOOP,
        skipped: 'could not reach :focus-visible with a real keyboard interaction; CDP pseudo-state forcing is not implemented yet',
    };
    if (before !== undefined)
        application.before = before;
    return application;
}
async function tabWalkTo(ctx, locator, steps) {
 ...
 */
async function pointOutside(ctx, box) {
    const viewport = ctx.page.viewportSize() ?? { width: 1280, height: 800 };
    const inside = (point) => point.x >= box.x && point.x <= box.x + box.width && point.y >= box.y && point.y <= box.y + box.height;
...
async function applyActive(ctx, steps) {
 ...
 */
async function applyFlagState(ctx, name, steps) {

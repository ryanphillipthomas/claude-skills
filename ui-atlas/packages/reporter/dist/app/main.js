...
function readModel() {
    ...
    if (holder === null)
        throw new Error('report data block is missing');
...
function matchesFilters(capture) {
    if (filters.routeKeys.size > 0 && !filters.routeKeys.has(capture.routeKey))
        return false;
    if (filters.viewports.size > 0 && !filters.viewports.has(capture.viewportLabel))
        return false;
    if (filters.states.size > 0 && !filters.states.has(capture.stateName))
        return false;
    if (filters.provenances.size > 0 && !filters.provenances.has(capture.provenance))
        return false;
    if (filters.statuses.size > 0 && !filters.statuses.has(capture.status))
        return false;
    if (filters.roles.size > 0) {
        const role = capture.element?.role;
        if (role === undefined || !filters.roles.has(role))
            return false;
    }
    if (filters.warningsOnly && capture.warnings.length === 0 && capture.error === undefined)
        return false;
...
/* Shared pieces                                                               */
/* -------------------------------------------------------------------------- */
function statusBadges(capture) {
    const out = [badge(capture.status, capture.status)];
    out.push(badge(capture.provenance, capture.provenance, capture.provenance === 'forced'
...
function shot(capture, className) {
    ...
    const explanation = capture.error?.code === 'locator.hidden'
...
function captureCard(capture) {
    const title = capture.element?.accessibleName ??
        capture.element?.textExcerpt ??
...
/* Views                                                                       */
/* -------------------------------------------------------------------------- */
function renderComponents(host, captures) {
    ...
    for (const group of groups)
        host.append(componentSection(group));
}
function componentSection(group) {
    const counts = [];
    if (group.capturedCount > 0)
        counts.push(badge(`${String(group.capturedCount)} captured`, 'captured'));
    if (group.skippedCount > 0)
        counts.push(badge(`${String(group.skippedCount)} skipped`, 'skipped'));
    if (group.failedCount > 0)
        counts.push(badge(`${String(group.failedCount)} failed`, 'failed'));
    const head = el('div', {
        ...
        headRow.append(el('th', {
            children: [
                ...
                sample === undefined
                    ? undefined
                    : el('small', {
                        text: `${String(sample.viewportWidth)}x${String(sample.viewportHeight)}` +
    ...
    for (const row of rows) {
        ...
        for (const column of columns)
            tableRow.append(matrixCell(cellFor(row, column)));
...
function renderGallery(host, captures) {
    ...
    for (const capture of captures)
        grid.append(captureCard(capture));
    host.append(grid);
}
function renderDuplicates(host, captures) {
    const groups = groupDuplicates(captures);
    if (groups.length === 0) {
        host.append(emptyState('No two captures in this selection share an image. Identical images usually mean a state that did not change anything.'));
        return;
    }
    for (const group of groups) {
...
function renderIssues(host, captures) {
    ...
    for (const capture of rows) {
        const label = capture.element?.accessibleName ?? capture.element?.tagName ?? `${capture.kind} · ${capture.routeKey}`;
    ...
    const head = el('thead', {
        children: [
            el('tr', {
                children: ['status', 'component', 'state', 'viewport', 'code', 'detail'].map((text) => el('th', { text })),
            }),
        ],
    });
...
function renderDetail(capture) {
    ...
    if (capture.verification !== undefined)
        overview.append(pair('evidence', capture.verification));
    overview.append(pair('kind', capture.kind));
    overview.append(pair('viewport', `${capture.viewportLabel} · ${String(capture.viewportWidth)}×${String(capture.viewportHeight)}` +
    ...
    if (capture.image !== undefined) {
        overview.append(pair('image', `${String(capture.image.width)}×${String(capture.image.height)} · ${formatBytes(capture.image.byteLength)}`));
        overview.append(pair('sha256', capture.image.sha256, true));
    }
    panel.append(section('Overview', overview));
    /* --- error / warnings -------------------------------------------------- */
    if (capture.error !== undefined) {
        panel.append(section(capture.status === 'skipped' ? 'Why this was skipped' : 'Why this failed', el('div', {
    ...
    if (capture.element !== undefined) {
        ...
        if (element.textExcerpt !== undefined)
            details.append(pair('text', element.textExcerpt));
        details.append(pair('fingerprint', element.fingerprint.slice(0, 24), true));
        if (element.frameDepth > 0) {
            details.append(pair('frame', `depth ${String(element.frameDepth)}${element.crossOriginFrame ? ' · cross-origin' : ''}`));
        }
        ...
        for (const candidate of element.candidates) {
            const chosen = candidate.type === element.chosen.type && candidate.value === element.chosen.value;
            const scoreClass = candidate.score === 0 ? 'score score--zero' : candidate.score >= 70 ? 'score score--high' : 'score score--low';
        ...
        panel.append(section('Locator candidates', el('table', { className: 'table', children: [head, body] }), el('p', {
            className: 'notelist',
    ...
    if (changed.length > 0) {
        ...
        const extras = [];
        if (capture.styleDelta?.descendantVisibilityChanged === true)
            extras.push('descendant visibility changed');
        if (capture.styleDelta?.boundsChanged === true)
            extras.push('bounds changed');
        panel.append(section('Computed style delta', el('table', { className: 'table', children: [head, body] }), extras.length === 0 ? undefined : el('p', { className: 'notelist', text: extras.join(' · ') })));
    }
    else if (capture.stateName !== 'default' && capture.status === 'captured') {
        panel.append(section('Computed style delta', el('div', {
    ...
    if (capture.readiness.checks.length > 0) {
        ...
        panel.append(section('Readiness', el('table', { className: 'table', children: [body] }), capture.readiness.deadlineExceeded
            ? el('div', {
                className: 'note note--warn',
...
function renderMain() {
    ...
    const captures = filteredCaptures();
    if (tab === 'components')
        renderComponents(host, captures);
    else if (tab === 'gallery')
        renderGallery(host, captures);
    else if (tab === 'duplicates')
        renderDuplicates(host, captures);
    else if (tab === 'issues')
        renderIssues(host, captures);
    else
        renderPages(host);
}
function select(id) {
    ...
    for (const node of Array.from(document.querySelectorAll('[data-capture]'))) {
        if (node.getAttribute('data-capture') === id)

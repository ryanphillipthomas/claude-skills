...
async function loadViewerBundle(): Promise<string> {
...
function renderDocument(model: ReportModel, viewer: string): string {
  ...
  if (model.unreadableRecords > 0) {
    notices.push(
      `<p class="note note--warn">${String(model.unreadableRecords)} record(s) in this run could not be read and are not shown.</p>`,
    );
  }

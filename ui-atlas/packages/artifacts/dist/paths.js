 ...
 */
export function sanitizeSegment(input, fallback = 'unnamed') {
    ...
    if (out.length === 0 || out === '.' || out === '..')
        out = fallback;
    if (RESERVED_NAMES.has(out.toLowerCase()))
        out = `${out}-x`;
    return out;
}
 ...
 */
export function routeKeyFromUrl(rawUrl) {
    let parsed;
    try {
...
export function toRecordPath(root, absolutePath) {

...
import { describeCandidate, locatorForCandidate, type LocatorRoot } from './locators.js';
...
function orderCandidates(identity: {
  ...
  const rest = identity.locatorCandidates.filter(
    (candidate) =>
      !(candidate.type === identity.chosenLocator.type && candidate.value === identity.chosenLocator.value),
  );
  return [identity.chosenLocator, ...rest];
}

function boxesMatch(a: Box, b: Box, tolerance: number): boolean {
  return (
...
export async function resolveElement(
  root: LocatorRoot,
  identity: Pick<ElementIdentity, 'chosenLocator' | 'locatorCandidates'>,
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    if (matches === 1) {
      ...
      if (index > 0 && options.expectedBox !== undefined) {
        ...
        if (box !== null && !boxesMatch(box, options.expectedBox, Math.max(tolerance, 4))) {
          warnings.push(
            `fallback candidate ${describeCandidate(candidate)} resolved to an element at a different position than the one that was selected`,
          );
        }
      }
      return {
    ...
    if (options.expectedBox !== undefined) {
      ...
      if (geometric !== undefined) {
        warnings.push(
          `disambiguated ${describeCandidate(candidate)} by position: used match ${geometric.index + 1} of ${matches}`,
        );
        return {
  ...
  throw new UiAtlasError(
    anyMatched ? 'locator.ambiguous' : 'locator.not-found',
    anyMatched
      ? 'no locator candidate resolved to a single element'
      : 'no locator candidate matched anything on the page',
    { detail: { warnings, candidates: candidates.map(describeCandidate) } },
  );
}

async function matchByGeometry(
  locator: Locator,
  expected: Box,
  tolerance: number,
  limit: number,
): Promise<{ locator: Locator; index: number } | undefined> {
  const found: Array<{ locator: Locator; index: number }> = [];
  for (let index = 0; index < limit; index += 1) {
    ...
    if (box !== null && boxesMatch(box, expected, tolerance)) found.push({ locator: nth, index });
...
export async function buildFramePath(frame: Frame): Promise<FrameIdentity[]> {
  const chain: Frame[] = [];
  ...
  for (let depth = 0; depth < chain.length; depth += 1) {
    ...
    if (depth > 0) {
      ...
      if (selector !== undefined) identity.selectorInParent = selector;
    }
    path.push(identity);
  }
  return path;
...
async function frameSelectorInParent(frame: Frame): Promise<string | undefined> {
  try {
    const handle = await frame.frameElement();
    const selector = await handle.evaluate((element: Element) => {
      const tag = element.tagName.toLowerCase();
      const name = element.getAttribute('name');
      if (name !== null && name.length > 0) return `${tag}[name="${name}"]`;
      const id = element.getAttribute('id');
      if (id !== null && id.length > 0 && !/^\d/.test(id)) return `${tag}#${id}`;
...
export function buildElementIdentity(probe: ElementProbe, framePath: FrameIdentity[]): ElementIdentity {
  ...
  if (probe.role !== undefined) identity.role = probe.role;
  if (probe.accessibleName !== undefined) identity.accessibleName = probe.accessibleName;
  if (probe.textExcerpt !== undefined) identity.textExcerpt = probe.textExcerpt;
  if (probe.shadowHostPath.length > 0) identity.shadowHostPath = probe.shadowHostPath;
  return identity;
}

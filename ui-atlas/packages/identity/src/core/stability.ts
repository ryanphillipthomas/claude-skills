...
export function inspectId(id: string): GeneratedIdVerdict {
  const trimmed = id.trim();
  if (trimmed.length === 0) return { generated: true, reason: 'empty id' };

  for (const pattern of GENERATED_PATTERNS) {
    if (pattern.test(trimmed)) return { generated: true, reason: `matches generated pattern ${String(pattern)}` };
  }

  if (trimmed.length > 40) return { generated: true, reason: 'id is unusually long' };
...
export function looksHashedClass(className: string): boolean {
  if (className.length > 30) return true;
  if (/^(?:css|sc|jsx|emotion|_)[-_]?[a-z0-9]{4,}$/i.test(className)) return true;
  if (/^[a-z0-9]{6,}$/i.test(className) && !HUMAN_WORD.test(className)) return true;
...
export function geometryBucket(width: number, height: number): string {
  const bucket = (value: number): string => {
    const rounded = Math.max(0, Math.round(value));
    for (const edge of BUCKET_EDGES) {
      if (rounded <= edge) return `<=${edge}`;
    }
    return '>2048';
  };
 ...
 */
export function normalizeNameClass(name: string | undefined): string {

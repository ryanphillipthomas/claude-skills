...
function ownerRoot(element: Element): Document | ShadowRoot {
  const root = element.getRootNode();
  if (root instanceof ShadowRoot) return root;
  return element.ownerDocument;
}

function topRoot(element: Element): Document {
  return element.ownerDocument;
}
...
function testIdAttribute(element: Element): { attribute: string; value: string } | undefined {
  for (const attribute of TEST_ID_ATTRIBUTES) {
    const value = element.getAttribute(attribute);
    if (value !== null && value.trim().length > 0) return { attribute, value: value.trim() };
  }
  return undefined;
}

function countDeep(selector: string, root: Document | ShadowRoot): number {
...
function simpleSelector(element: Element): string {
  ...
  if (classes.length > 0) return `${tag}${classes.map((c) => `.${cssEscapeIdent(c)}`).join('')}`;
  return tag;
}
 ...
 */
export function cssPathFor(element: Element): string {
  const parts: string[] = [];
  ...
  while (current !== null && depth < 12) {
    ...
    parts.unshift(
      sameTagSiblings.length > 1 ? `${tag}:nth-of-type(${nthOfTypeIndex(current)})` : tag,
    );
    current = parent;
    depth += 1;
  }
...
function findScopeAnchor(element: Element): ScopeAnchor | undefined {
...
export function buildCandidates(element: Element): CandidateDraft[] {
  ...
  if (role !== undefined && accessibleName !== undefined && accessibleName.length > 0) {
    ...
    for (const candidate of matches) {
      if (computeRole(candidate) !== role) continue;
      if (computeAccessibleName(candidate, role) === accessibleName) count += 1;
    }
    drafts.push({
      type: 'role-name',
  ...
  if (text.length > 0 && text.length <= 80) {
    ...
    let count = 0;
    for (const candidate of matches) {
      if (directText(candidate) === text) count += 1;
    }
    drafts.push({
      type: 'text',
      value: text,
...
function countLabelled(doc: Document, labelText: string): number {
  let count = 0;
  for (const label of Array.from(doc.querySelectorAll('label'))) {
    if (collapseWhitespace(label.textContent ?? '') !== labelText) continue;
...
export function buildFingerprintInput(element: Element): FingerprintInput {
  const role = computeRole(element);
  const accessibleName = computeAccessibleName(element, role);
  ...
  const testId = testIdAttribute(element);
  if (testId !== undefined) stableAttributes[testId.attribute] = testId.value;
  if (element.id.length > 0 && !inspectId(element.id).generated) stableAttributes['id'] = element.id;
  const type = element.getAttribute('type');
  if (type !== null) stableAttributes['type'] = type.toLowerCase();
  const name = element.getAttribute('name');
  if (name !== null && !inspectId(name).generated) stableAttributes['name'] = name;

  const ancestorRoles: string[] = [];
  for (const ancestor of composedAncestors(element)) {
    const ancestorRole = computeRole(ancestor);
    if (ancestorRole !== undefined) ancestorRoles.push(ancestorRole);
    if (ancestorRoles.length >= 6) break;
  }
...
function reportedAttributes(element: Element): Record<string, string> {
...
function looksLikeClosedShadowHost(element: Element): boolean {
  return (
    element.tagName.includes('-') && element.shadowRoot === null && element.childElementCount === 0
  );
}

export function probeElement(element: Element): ElementProbe {
  const role = computeRole(element);
  const accessibleName = computeAccessibleName(element, role);
  const rect = element.getBoundingClientRect();
  ...
  if (role !== undefined) probe.role = role;
  if (accessibleName !== undefined) probe.accessibleName = accessibleName;
  if (text.length > 0) probe.textExcerpt = excerptText(text);
  return probe;
}

...
import type { Box, ReadinessCheck, ReadinessCheckName, ReadinessResult } from '@ui-atlas/protocol';
...
import {
  ...
  readMutationQuiet,
  waitAnimationFramesInPage,
  waitForFonts,
  type MutationQuietState,
...
class CheckRecorder {
  ...
  record(name: ReadinessCheckName, status: ReadinessCheck['status'], startedAt: number, detail?: string): void {
    const check: ReadinessCheck = { name, status, durationMs: Math.max(0, Date.now() - startedAt) };
    if (detail !== undefined) check.detail = detail;
    this.checks.push(check);
    if (status === 'timed-out') this.warn(`${name} timed out${detail === undefined ? '' : `: ${detail}`}`);
    if (status === 'failed') this.warn(`${name} failed${detail === undefined ? '' : `: ${detail}`}`);
  }
  ...
  snapshot(): { checks: ReadinessCheck[]; warnings: string[] } {
 ...
 */
export async function settlePage(page: Page, options: SettleOptions): Promise<ReadinessResult> {
  ...
  if (options.target !== undefined) await runElementStable(options.target, config, deadline, recorder);
  if (options.skipMutationQuiet !== true) await runMutationQuiet(page, config, deadline, recorder);
  ...
  if (deadlineExceeded) warnings.push(`settle hit its ${totalMs}ms deadline; captured anyway`);
...
async function runFonts(
  ...
  try {
    ...
    if (outcome === TIMED_OUT) recorder.record('fonts-ready', 'timed-out', startedAt);
    else recorder.record('fonts-ready', 'passed', startedAt, outcome);
  } catch (error) {
...
async function runImages(
  ...
  try {
    const outcome = await withTimeout(
      page.evaluate(decodeVisibleImages, config.perImageTimeoutMs),
      budget,
    );
...
async function runElementStable(
  target: Locator,
  config: SettleConfig,
  deadline: Deadline,
  recorder: CheckRecorder,
...
async function runMutationQuiet(
  page: Page,
  config: SettleConfig,
  deadline: Deadline,
  recorder: CheckRecorder,
  ...
  while (!deadline.expired()) {
    ...
    if (state.quietForMs >= config.mutationQuietMs) {
      recorder.record(
        ...
        'passed',
        startedAt,
        `${config.mutationQuietMs}ms quiet after ${state.mutationCount} mutations`,
      );
      return;
    }
...
async function runAnimationFrames(
  ...
  try {
    const outcome = await withTimeout(
      page.evaluate(waitAnimationFramesInPage, config.animationFrames),
      budget,
    );
    if (outcome === TIMED_OUT) recorder.record('animation-frames', 'timed-out', startedAt);
    else recorder.record('animation-frames', 'passed', startedAt, `${config.animationFrames} frames`);
  } catch (error) {
...
export async function disposeSettle(page: Page): Promise<void> {
  try {
...
export async function waitAnimationFrames(page: Page, count = 2): Promise<void> {
  try {

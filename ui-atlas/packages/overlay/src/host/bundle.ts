...
import { UiAtlasError, type OverlayBootstrap } from '@ui-atlas/protocol';
 ...
 */
export function overlayBundlePath(): string {
  return fileURLToPath(new URL('../../dist/page-bundle.js', import.meta.url));
}

export async function loadOverlayBundle(): Promise<string> {
  const path = overlayBundlePath();
  if (!existsSync(path)) {
    throw new UiAtlasError(
      'internal',
      'the overlay page bundle is missing; run `npm run build:overlay`',
 ...
 */
export function buildBootstrapScript(bundle: string, bootstrap: OverlayBootstrap): string {
  return [
    '(() => {',

...
import { OVERLAY_HOST_ATTRIBUTE, type HostEvent } from '@ui-atlas/protocol';
import {
  dispatchToOverlay,
  hideOverlayHosts,
  isOverlayMounted,
...
export class OverlayController {
  ...
  async dispatch(event: HostEvent): Promise<void> {
  ...
  async broadcast(event: HostEvent): Promise<void> {
    await Promise.all(
      this.page.frames().map((frame) => frame.evaluate(dispatchToOverlay, event).catch(() => false)),
    );
  }
  ...
  async waitForMount(timeoutMs = 10_000): Promise<boolean> {
    const started = Date.now();
    for (;;) {
      if (await this.isMounted()) return true;

...
name: prompt-master
version: 1.7.0
description: Generates optimized prompts for AI tools. Use in Codex or ChatGPT when the user explicitly asks to write, fix, improve, or adapt a prompt for a specific AI tool (ChatGPT, Codex, another LLM, Cursor, Midjourney, image AI, video AI, coding agents, etc.). Does not activate for general conversation, coding tasks, document writing, or other non-prompt-engineering work.
---
...
- Treat Codex as an agentic coding environment, not a plain chat model
- State the outcome, repository or file scope, constraints, allowed actions, and what "done" means
- Explicitly request tool use, tests, or verification when the task depends on them
- Include approval gates for destructive actions, dependency installation, external writes, or schema changes
- Avoid prescribing internal reasoning or step-by-step thought; specify observable checks and final evidence instead
- Add: "Only make changes directly required by this task. Preserve unrelated user changes."
- Use Template H for autonomous execution and Template M for complex multi-file work
